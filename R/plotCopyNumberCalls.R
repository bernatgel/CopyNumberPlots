...
plotCopyNumberCalls <- function(karyoplot, cn.calls, cn.colors=NULL, loh.color="#1E90FF", labels="", lab.cex=1, lab2.cex=NULL, loh.height=0.3, r0=0, r1=1) {
  ...
  segments <- subsetByOverlaps(segments, plot.region)
  
  seg.cols <- segment.colors[as.character(segments$cn)]
  
  
  kpRect(kp, data=segments, y0=loh.height, y1=1, col=seg.cols, r0=r0, r1=r1, border=NA)
  kpRect(kp, data=segments[segments$loh==1], y0=0, y1=loh.height, r0=r0, r1=r1, col=loh.color, border=NA)
  if(!is.null(labels) && !is.na(labels) && all(is.character(labels) && length(labels)>0)) {
    if(length(labels)==1) {
      ...
      kpAddLabels(kp, labels = label[2], r0=r0, r1=r0+(r1-r0)*loh.height, cex=lab2.cex)  
    }
  }
  invisible(karyoplot)
}
