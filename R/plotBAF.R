...
#'
#' @usage plotBAF(karyoplot, snps, baf.column="baf", label="BAF", points.cex=0.3, points.col="#333333", points.pch=16, label.cex=1.5, label.srt=90, label.margin=0.03, add.axis=TRUE, axis.cex=1.2, r0=0, r1=1, data.panel=1, autotrack=NULL, ...) {
#'
#'
...
#'
 ...
 #TODO: Extend examples with data.panel and autotrack?

plotBAF <- function(karyoplot, snps, baf.column="baf", label="BAF", points.cex=0.3, points.col="#333333", points.pch=16, label.cex=1.5, label.srt=90, label.margin=0.03, add.axis=TRUE, axis.cex=1.2, r0=0, r1=1, data.panel=1, autotrack=NULL, ...) {
  ...
  if(!is.null(label) && !is.na(label) && nchar(label)>0) {
    kpAddLabels(karyoplot, r0=r0, r1=r1, labels = label, srt=label.srt, pos = 3, cex=label.cex, label.margin = label.margin, data.panel=data.panel, autotrack=autotrack, ...)
  }

  if(add.axis==TRUE) {
    kpAxis(karyoplot, r0=r0, r1=r1, ymin=0, ymax=1, cex=axis.cex, data.panel=data.panel, autotrack=autotrack, ...)
  }

  kpPoints(karyoplot, data=snps, y=mcols(snps)[,baf.column], ymin=0, ymax=1, r0=r0, r1=r1, col=points.col, pch=points.pch, cex=points.cex, data.panel=data.panel, autotrack=autotrack, ...)
